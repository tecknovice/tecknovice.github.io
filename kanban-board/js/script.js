...
let app = {
    ...
    insertIntoList(todo, type) {
        let text = `<li class="list-group-item">
                        <div>${todo}</div>
                        <i class="fas fa-trash-alt " onclick="app.deleteTodo(this)"></i>
                    </li>`
        $('#' + type).append(text)
    },
    deleteTodo(icon) {
...
$(function () {
    ...
    $(".list-group").sortable({
        ...
        start: function (event, ui) {
            ui.item.oldType = ui.item.context.parentElement.getAttribute('id')
        ...
        stop: function (event, ui) {
            ...
            let newType = ui.item.context.parentElement.getAttribute('id')
            let newIndex = ui.item.index()
            console.log(oldType, oldIndex, newType, newIndex)
